...
foreach ($result->HamtaListaResult->ObjektUppdateringsinfo as $key => $value) {
	...
	if($value->Typ == 'CMNyProd' || $value->Typ == 'CMNyprod')
	{
		...
		$check_guid = "SELECT anvandare_id,guid,senastandrad FROM  `".$table_name."` where `guid`='".$value->Guid."'";
		...
		if ($guid_result->num_rows > 0)
		{
			...
			{
				echo "GUID = ".$value->Guid."<br>";
				echo "old date = ".$upd_date."<br>";
				echo "new date = ".$HamtaProjektResult->SenastAndrad."<br>";
				$qu=true;
			...
			else
			{
				...
				echo "old date = ".$upd_date."<br>";
				echo "new date = ".$HamtaProjektResult->SenastAndrad."<br>";
				echo "exists and don't do anything.<br>";
				$qu=false;
				echo "<hr>";
			}
		}
