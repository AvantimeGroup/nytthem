...
function load_files($url,$main_guid,$child_guid,$folder){
...

	{
		...
		//echo 'http://fastighet.capitex.se/'.$url , $dirpath;
		
		if(!file_exists($dirpath.'/pdf_'.$child_guid.'.pdf'))

		{

			@copy( $url , $dirpath.'/pdf_'.$child_guid.'.pdf');
...
function load_imagetype($filtyp,$url,$main_guid,$child_guid,$folder){
...

	{
		...
		if(!file_exists($dirpath.'/img_'.$child_guid.'.'.$filtyp.''))

		{

			@copy( $url , $dirpath.'/img_'.$child_guid.'.'.$filtyp.'');
		  ...
		  /*  echo '<pre>';
		   ...
		   echo '</pre>';  */
   			...
   			foreach($BostadRestult as $bostad_key=>$bostad_val)
			{
				...
				// if($bostad_key=='Adress'  || $bostad_key=='Vagbeskrivning' || $bostad_key=='Projektnamn'
				// 	 || $bostad_key=='RumsbeskrivningFritext'  || $bostad_key=='Beskrivning'  || $bostad_key=='Visningar'
				// 	  || $bostad_key=='SaljandeBeskrivningar' || $bostad_key != "Bilder")
				...
				if($bostad_key=="Filer")
					{
					//	$bostad_val = serialize($bostad_val);
						...
						if(is_array($bostad_val1))
						{
							foreach($bostad_val1 as $filer)	
							{
								...
								load_files($url,$p_guid,$c_guid,"filer");
							}
						}
						else{
...
//print_r($result->HamtaListaResult->ObjektUppdateringsinfo);
